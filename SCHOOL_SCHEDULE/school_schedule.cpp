...
void checkSchedule(String currentTime, int currentDay) {
  ...
  unsigned long currentTimeInSeconds = timeToSeconds(currentTime);  // Convert current time to seconds
  ...
  while (scheduleFile.available()) {
    ...
    unsigned long eventTimeInSeconds = timeToSeconds(eventTime);
    ...
    // Check if the current time matches and the day is valid
    if (currentTime == eventTime && eventDays.indexOf(String(currentDay)) >= 0) {
